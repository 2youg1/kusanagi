...
const table = [_]Entry{
    ...
    .{ "room_switch", "A room, not a channel: everybody invited reads everybody", "建房间而不是通道:受邀的每个人都能读到彼此" },
    .{ "room_hint", "Members learn each other's handles; only you, the founder, can invite. There is no removing anybody.", "成员会互相知道彼此的标识;只有你(建房者)能邀请。没有踢人。" },
    .{ "join_room_switch", "This is a room invitation", "这是房间邀请" },
    .{ "cancel", "Cancel", "取消" },
    .{ "mint", "Mint", "生成" },
    ...
    .{ "reached", "reached", "已送达" },
    .{ "room_badge", "room", "房间" },
    .{ "room_is_shared", "A room is shared", "房间是共享的" },
    .{ "room_body", "Everybody here reads everybody. Whatever you write lands once, on your own stream, and every member's next read picks it up.", "这里的每个人都能读到彼此。你写的话只落一次,在你自己的流上;每个成员下一次读就能看到。" },
    ...
    .{ "note_room_founded", "room founded; minting the invitation", "房间已建,正在生成邀请" },
