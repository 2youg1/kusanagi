...
-- and be told to paste the whole `kusanagi2:` line, which sends a confused
...
-- that searched for @kusanagi2:@ concluded that no invitation had been offered —
...
adviceIsAboutWhatWasGiven arguments complaint
  ...
  | otherwise = Right ()
  where
    mentionsInvitation = Text.isInfixOf "kusanagi2:" (complaintRecover complaint)
    suppliedInvitation =
      "join" `elem` arguments || any (Text.isInfixOf "kusanagi2:" . Text.pack) arguments
