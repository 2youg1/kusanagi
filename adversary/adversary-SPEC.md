...
| 宿主是什么 | 一个本地目录，分片规则为地址前两字符 | 换 HTTP 盒子时该细节消失，`Ground.hs` 的敌意动作改为 HTTP 请求 |
...
**这条阈值能抓到什么、抓不到什么，写清楚以免被当成更强的声明。** 它抓的是散布在整个 drop 上、量级超过 5σ 的重合：重用的密钥流、恒定 nonce、未加密的尾巴。它抓不到一个很短的固定字段——比如每个 drop 相同位置上的四字节 tag，`DROP = 131 072` 时它只把重合抬高 4，远在 113 的噪声底之下。首部头由 `prefixTolerance` 单独卡住；**中间的短固定字段今天没有任何性质在看**。能看它的是另一条性质：比的不是一对 drop 重合多少，而是哪些**位置**在多对上同时重合——碰巧是散的，结构是对齐的。它是一条新性质，不在本次改动里。
