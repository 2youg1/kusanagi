...
13. **有没有人说话，只按写下来的那份清单分离。** 沉默的世界对忙碌的世界，分离集合与
    `Tempo.declared` **相等**。这条问的是**按需通道**，`declared = {gap.burst}` 至今仍是
    实测事实；**等式两边都咬人**——一个开始分离的特征是新泄漏，一个停止分离的特征是
    有人建了机制而没改这行。
14. **时隙通道上，那份清单是空集。**（I3）沉默世界与忙碌世界在**两个位置上都不分离**：
    载运者听到的时序（`Tempo.presenceSaysNothingOnASlottedChannel`）与宿主数到的对象数
    （`volumeSaysNothingOnASlottedChannel`）。两个世界排队的正文条数不同、`tick` 次数相同，
    **那个不对称就是实验本身**：流量必须跟着排班走，不跟着队列走。
    两个世界各 tick 两次，于是第二次发现时隙已填而不写——**一个一周期内触发两次的排班器
    必须只产出一个 drop**，这一条也在这里被量而不是被假设。
...
| `Tempo.declared` = `{gap.burst}`，**只对按需通道** | 实测：沉默世界 3、忙碌世界 12，四个世界零方差。`gap.median` 一次也没分离——两边每条命令付同一份进程启动 | I3 已落地，但它是**按通道选入**的：一条 `--every` 通道的清单是空集（新增两条性质各守一个位置），而一条按需通道的清单一字不改。所以这一行留着，且必须留着——它描述的是今天大多数通道的真实状态 |
| 时隙实验的周期取 **3600 秒**，两个世界各 `tick` 两次 | 周期短会让实验去量时钟：两次 tick 之间隔多久成了变量。一小时保证第二次 tick 必然落在同一时隙里，于是被量到的是**排班的产物**——一个时隙一个 drop——而不是墙上时钟的抖动 | 若将来要量「跨时隙」的行为（例如队列排空的节奏），那是另一个实验，需要另一个周期和一次显式等待，不要改这一个 |
| `--for` 只取 `0` 与 `3600` | 见 §3 | —— |
