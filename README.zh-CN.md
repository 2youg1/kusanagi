...
| 让主机无法把写者与读者配对 | 今天一次轮询点名一个地址，主机看得到谁写了它、谁来要它。下一版列举整个 bin 并全部读取，读请求只是公开信息的函数（Roadmap D-20）。 |
| 隐藏端点的 IP 地址 | 不归这个项目解决。把 `KUSANAGI_PROXY` 指向 SOCKS5 或 HTTP CONNECT 代理，让为此而生的网络去做；`kusanagi proxy --require` 让代理缺席变成拒绝，而不是直连。 |
| 隐藏哪些 channel 共用一把桶凭据 | S3 的 access key 随它签的每个请求一起走，桶的日志因此把一把钥匙写过的一切连在一起。`kusanagi host` 不要凭据，也就没有这条边。 |
