...
| `name [--as 名字 \| --clear]` | 说出你想被怎么称呼。名字由你的密钥签名，密封在此后你发出的每一份邀请与问候里；对端在你的 handle 旁看到它，并能核对它确属于你。它是标签不是证明——当面核对的仍是 handle 与校验码。设名之前认识的对端看不到变化。 |
...
| 哪个读者要的是哪个对象 | **藏在同一个 ward 的读者之中。** 同一个 ward 的每个读者发的请求都一样，主机能说某个 ward 被读过，说不出是谁为了什么。读者把扫描的前缀缩短，就以带宽换取藏进更多 ward。 |
| 一共存了多少个对象 | **看得到。** |
| 每个对象多大 | **看不到。** 每个 drop 恒为 131 072 字节，无论装什么。 |
| 每次请求什么时候到达 | **看得到。** |

读取方若向主机点名一个地址，就把这个网络最想藏的那一对关系——谁写了这个地址、谁来取它——直接交到了主机自己的访问日志上，不需要任何密码分析。所以读取方从不点名地址。每个 drop 归档在一个公开的十分钟周期和读者的 **ward** 之下——ward 是读者建身份时随机选定的一个号码，交给每个要写给它的人——一次读取列举自己 ward 的一个周期，取走列举里上次没见过的对象，在自己的机器上按地址匹配。主机看到的是一个 ward 被读了，每个读它的人做法都一样；它永远看不到其中哪个对象是被要的。取回而没匹配上的东西不会保存。

代价说在明处：一个忙碌的 ward 让它的读者多付带宽，因为每个人都要取走这个 ward 里所有人收到的东西；一个 bin 超过 256 个对象会被拒绝而不是读取（`kusanagi.ward_overfull`）；写者的时钟若比读者慢十分钟以上、跨周期写入，那一段会落在读者已经看过的 bin 里，要等那个 bin 下一次变化才被取到。
