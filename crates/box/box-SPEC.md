...
选择释放的通道需要它：对端确认之后 drop 被移除，于是一个**老实的**宿主手上没有历史可被调取。

**应答与其他一切一样，是那个恒定的空 `404`。** 一个能区分「删掉了」与「本来就没有」的应答，
就是交给扫描者一台地址预言机——这正是这个文件里其余每一条拒绝所要否认的东西。

**知道地址的人就可以删。** 地址由通道秘密派生，能算出一个地址就已经等于持有这个 drop 所保护的
一切；而一个要判断「你是不是有权删」的宿主，必须先知道你是谁。一个有敌意的删除者能做的是销毁字节，
而那件事任何宿主本来就做得到——它是宿主。
