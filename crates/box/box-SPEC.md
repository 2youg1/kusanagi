...
| U4 过期 | `Cache-Control: max-age=<秒>` 落成对象前缀的到期戳 | 写入时已过期的对象永远读不到 |
| U6 不自报家门 | 无横幅、无状态路径、拒绝一律空正文、对不持地址者只有一种回答 | 扫描者的任意一组请求得到**逐字节相同**的响应，且没有一个响应含有本项目的名字（`tests/unmarked.rs`） |
| U7 恶意调用者 | 近似拼写的条件头、溢出的寿命、大写地址、谎报的长度 | 四条规则各自成立（`tests/hostile.rs`） |
| U5 有界 | 请求头 8 KiB、正文 1 MiB、空闲 30 秒、总量 1 GiB | 陌生人无法让本进程按他的说法分配内存，也无法把别人的磁盘填满 |
| U6 写入不被确认 | 每一个 `PUT` 都回同一个空 `404` | 一次 `PUT /d/<40 位十六进制>` 不再等于「我找到了一个 box」；全网扫描不再等于一份宿主名单 |
...
`Content-Length` 先按 `u64` 解析并与上限比较，再转成长度——声明 2⁶³ 字节的请求在分配之前就被拒；
出现两次或含非数字字符一律 400（`tests/robust.rs`）。

## 11 边界枚举

| 情形 | 期望 |
|---|---|
| `GET /health`、`GET /`、任何不是 drop 的路径 | 404 空正文，与下一行**逐字节相同** |
| `GET` 一个合法但不存在的地址 | 404 空正文 |
| `GET` 已过期的对象 | 404，字节仍在盘上但永不出门 |
| 大写十六进制地址 | 404。地址只有一种拼写，否则重拼一遍就绕过了写一次语义 |
| `PUT` 缺 `If-None-Match: *`，或写成 `"*"`、`W/*`、`**` | 428，且地址仍未被占 |
| `PUT` 到已被占用的地址 | 412 |
| `Cache-Control` 无法解析 | **忽略**（RFC 9111 §5.2），不报错；报错就把一个畸形值变成了区分本宕主与一个缓存的手段 |
| `max-age` 大到加不动 | 饱和，不回绕；回绕会把对象的到期时刻拉到现在之前 |
| 请求头超过 8 KiB / 正文超过 1 MiB | 400 空正文，在分配之前 |
| `Content-Length` 出现两次 | 400。一个要求读者去挑一个的请求，正是「一个请求变成两个」的做法 |
| `Content-Length` 不是纯数字（`+8`、`-1`、`0x10`） | 400。**Rust 的整数解析接受前导 `+`，HTTP 不接受**；两个读者对长度的理解不一致就是走私 |
| 非 GET/PUT 方法 | 404，不是 405。405 告诉对方本主机实现了哪些方法 |
| 畸形请求 | 是一个响应，不是一个错误；且响应不携带畸在哪里 |
...
|---|---|---|
| `MAX_HEAD = 8 KiB` | 头部有界 | —— |
| `CAPACITY = 1 GiB`，`kusanagi host --cap` 可改 | 一次写对发送者是一个请求，对宿主是一块磁盘。1 GiB 约八千个 drop：比任何两个人会交换的都多，又小到「填满它」得是个项目而不是一个下午 | 每次写前走一遍目录求和——计数器是状态，而被 kill 的宿主会丢掉状态；一个 box 有几千个文件，不是几百万 |
| 正文上限 = `kusanagi_waypoint::MAX_OBJECT`（1 MiB） | 正文有界，远高于一个段 | **本 crate 不再自己写这个数**：客户端愿意读回的上限与宿主愿意接受的上限是同一个数，两处各写一遍就会漂。`content-length` 先按 `u64` 解析、先比较再转成长度——声明 2⁶³ 字节的请求在分配之前就被拒 |
| `IDLE = 30s` | 空闲连接断开 | —— |
| 每连接一线程 | 最简单且不会死锁 | 有真实流量后再测量 |

**本主机没有横幅、没有版本、没有状态路径。** 一个存在于众所周知路径上、对任何人都回答「是，这里是 kusanagi」的接口，等于把一次全网扫描变成一份本网络的用户名单。宿主是被实测的：`doctor` 写两次读一次，那才是证据。
