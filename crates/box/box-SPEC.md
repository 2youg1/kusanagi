...
**已删除的硬编码：`BANNER = "kusanagi-box/1 …"` 与 `GET /health`。** 它从来不是证据——`doctor` 一直无视它并实测——而它是什么，是一个存在于众所周知路径上、对任何人都回答「是，这里是 kusanagi」的一次请求。删它没有代价：全 workspace 唯一的调用方是断言它自己文本的那个测试。
