...
| U7 只有主人能读 | `permissions`：本 crate 写盘的唯一入口 | Unix 上站点里没有任何文件或目录对其他账号可读；旧构建留下的宽权限文件在下一次写入时被改回来 |
...
| 替换旧记录用「旁边暂存 + 重命名盖过去」 | 不是为了原子性而已。`rename` 作用于**名字**，因此被替换的是链接本身而不是它指向的东西；新文件是新 inode，生下来就是 `0600`，于是「修正旧构建的文件」这个动作根本不存在。与 `waypoint::dir` 让一个 drop 整个出现用的是同一个形状 |
| 旧构建建的**目录**保持原模式 | 里面每个文件仍是 `0600`，因此暴露的是通道名的集合而不是内容。关上它需要 chmod 一个本构建未曾创建的目录，那是上一行那个操作换了顶帽子 |
