...
`seal` / `open` 现在取一个 `Fit`：`Veil` 填充到 `DROP`，`Exact` 不填充。**枚举不是布尔，
因为两者是两种不同的承诺**——被遮蔽的体积对全网每条消息都一样，那正是宿主读不出「说了多长」的
原因；精确体积只在没有任何不可信方看得到的地方才安全。目前 `Exact` 恰好一个用处：
`kusanagi-site` 的备份归档，它去的是主人自己选的一个文件，不去任何宿主。
