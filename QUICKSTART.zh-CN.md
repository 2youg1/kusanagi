...
- **invitation（邀请）**——一行很长的文本（文件夹主机约 180 字符，地址主机更长）。拿到它的人就能加入，所以递它的方式要像递钥匙。
...
绝对路径会原样进邀请行，`invite` 会大声说出来——这行只递给同在这台机器上的人。
...
**多少钱？** 现成的文件夹或自己跑的 `kusanagi host`，按零算。收费的桶收列举和下载的钱。
